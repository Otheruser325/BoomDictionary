...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            const filter = i => i.customId === 'submit-report' && i.user.id === interaction.user.id;
            ...
            collector.on('collect', async (i) => {
                ...
                const modalFilter = (modalInteraction) => modalInteraction.customId === 'report-form' && modalInteraction.user.id === interaction.user.id;
                interaction.client.once('interactionCreate', async (modalInteraction) => {
                    ...
                    const developerUser = await i.client.users.fetch(developerId);
            ...
            collector.on('end', async (collected, reason) => {
                if (reason === 'time' && collected.size === 0) {
                    try { 
                        ...
                        if (error.code === 10008) {
                            return interaction.followUp(`The submit embed was deleted and couldn't be recovered, please try again later.`);
            ...
            if (error.code === 10008) {
                return interaction.followUp(`I couldn't fetch the data for the interaction ${interaction.customId || 'this interaction'}, please try again later.`);
            } else if (error.code === 10062) {
                return interaction.followUp("My systematic networking is currently out of sync and timed out. Please try again later.");
            } else if (error.code === 40060) {
                return interaction.followUp("I couldn't reuse this interaction as I've already acknowledged it. Please try again later.");
                ...
                interaction.reply('An error occurred while executing the submit command. Please try again later.');
            }
        }
    }
};