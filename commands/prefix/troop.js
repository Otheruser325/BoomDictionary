        ...
        try {
            if (args.length === 0) {
                ...
                troopCollector.on('collect', async (interaction) => {
                    ...
                    const levelRow = new ActionRowBuilder().addComponents(troopLevelSelectMenu);
                    ...
                    levelCollector.on('collect', async (interaction) => {
                        if (interaction.customId !== 'select-troop-level') return;
                        ...
                        const [selectedTroopType, level] = interaction.values[0].split('-');
                        ...
                        } else if (selectedTroopType === 'grenadier' || selectedTroopType === 'bombardier') {
                            embedTroop.addFields({
                                name: 'Health',
                                value: formatNumber(stats.health),
                                inline: true
                ...
                troopCollector.on('end', async (collected, reason) => {
                    if (reason === 'time' && collected.size === 0) {
                        await reply.edit({
                            content: 'You did not select a troop type in time. Please try again.',
                            embeds: [],
                            components: []
                        });
                    }
